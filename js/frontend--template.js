...
(function () {
	...
	function addFiles(fileList, field, multiple) {
		...
		if (multiple) {
			...
			// .filter((file) => {
			// 	const maxSize = +input.getAttribute('data-size');
...
function initPhoneMask() {
	...
	// console.log(document.querySelector('input[type="year"]'));
	// $('input[type="year"]').mask('9999', {
	// 	placeholder: separator
	// });
}
...
(function () {
	...
	//сохранить фильтр пользователя
	$('body').on('click', '.-save-user-filter-form-add-', function () {
		let p35 = $('#select-id-35 option:selected').text(); //Тип объекта
		let p10 = $('#select-id-10 option:selected').text(); //Тип дома
		let p11MAX = $("#MAIN_FILTER_11_MAX").val(); //Площадь дома ДО
		let p20MAX = $("#MAIN_FILTER_20_MAX").val(); //Цена ДО

		$.fancybox.open({
			src: encodeURI('/local/ajax/modal_save_user_filter_form.php?house=' + p35.trim() + '&type=' + p10.trim() + '&pricemax=' + p20MAX + '&shousemax=' + p11MAX),
			type: 'ajax',
			touch: false,
			baseTpl: jQuery.fancybox.defaults.ajaxTpl,
		});
...
function user_filter_find() {
	let TYPEOBJ = $('#select-id-35 option:selected').text();
	let TYPEHOUSE = $('#select-id-10 option:selected').text();
	let PRICE = $('#MAIN_FILTER_20_MAX').val();
	let S_HOUSE = $('#MAIN_FILTER_11_MAX').val();

	console.trace(TYPEHOUSE, TYPEOBJ, S_HOUSE, PRICE);
	$.ajax({
		url: '/local/ajax/user-filter-find.php',
		data: {
			'TYPEHOUSE': TYPEHOUSE, 'TYPEOBJ': TYPEOBJ, 'PRICE': PRICE, 'S_HOUSE': S_HOUSE
		},
		success: function (json) {

			console.log(json);
			if (json.status) {
				$('#userfilerform').html('<div class="filter-form__link filter-form__remove "> <button type="button" class="link save-user-filter-form -saved-search-item__remove-" data-action="remove" data-modal-link="delete-saved" data-id="' + json.id + '"> <svg class="icon"> <use xlink:href="/local/img/sprite.svg#delete" /></svg> Удалить поиск </button> </div>');

			} else {
				$('#userfilerform').html('<div class="filter-form__link "> <a class="link save-user-filter-form -save-user-filter-form-add-" data-action="add"> <svg class="icon"> <use xlink:href="/local/img/sprite.svg#like-search" /></svg> Сохранить фильтр и оповещать о новых объектах </a> </div>');
			}
		},
		dataType: 'json'
	});
...
window.addEventListener('load', () => {
	...
	document.addEventListener('change', async (event) => {
		...
		if (event.target.closest('#select-id-35')) { user_filter_find() }
		if (event.target.closest('#select-id-10')) { user_filter_find() }
		if (event.target.closest('#MAIN_FILTER_20_MAX_pseudo')) { user_filter_find() }
		if (event.target.closest('#MAIN_FILTER_11_MAX_pseudo')) { user_filter_find() }
	})
